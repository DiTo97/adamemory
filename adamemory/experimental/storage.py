...
class STM:
    ...
    def add_memory(self, data: Dict[str, Any]):
        node = self.graph.add_node(
            data["label"], {**data["properties"], "user_id": self.user_id}
        )
    ...
    def search_memory(self, query: Dict[str, Any]) -> List[Dict[str, Any]]:
        return self.graph.search_nodes(
            query["label"], query["properties"], self.user_id
        )
...
class LTM:
    ...
    def add_memory(self, data: Dict[str, Any]):
        node = self.graph.add_node(
            data["label"],
            {**data["properties"], "recency_weight": 1.0, "user_id": self.user_id},
        )
        if "relations" in data:
            for relation in data["relations"]:
                self.graph.add_edge(
    ...
    def consolidate_memory(self, stm_memories: List[Dict[str, Any]]):
        for memory in stm_memories:
            existing_nodes = self.graph.search_nodes(
                memory["label"], memory["properties"], self.user_id
            )
            if existing_nodes:
                # Update the existing node's recency weight and properties
                node_id = existing_nodes[0]["n"].id
                self.graph.update_node_property(
                    memory["label"],
                    node_id,
                    {"recency_weight": existing_nodes[0]["n"]["recency_weight"] + 1},
                )
            else:
    ...
    def _prune_graph(self):
        weight_threshold = (
            0.5  # This threshold can be adjusted based on desired retention
        )
