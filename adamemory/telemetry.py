...
g_anonymous_id = config["DEFAULT"]["anonymous_id"]  # Retrieve the anonymous ID from config
...
def _send_event_json(event_json: dict):
    ...
    try:
        ...
        req = request.Request(TRACK_URL, data=data, headers=headers)  # Create the HTTP request
...
def send_event_json(event_json: dict):
    ...
    if not g_telemetry_enabled:
        raise RuntimeError("Telemetry tracking is disabled!")  # Raise an error if telemetry is disabled
    try:
        ...
        try:
            ...
            if is_telemetry_enabled():
                try:
                    ...
                    capture_event("function_usage", {"function_name": function_name})  # Log the function usage
                except Exception as e:
